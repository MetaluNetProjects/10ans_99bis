...
void main (void)
{
	...
	vec2 ncoord = coord ;/// 1024;
	coord2 = ncoord * 2.0 - vec2(1.0);
	
#ifdef DORECT
	polarCoord.y = length(coord2);
	polarCoord.y = (polarCoord.y - rMin) / (rMax - rMin);

	if(rect >= 0.0) {
		rectCoord.y = ncoord.x;
		rectCoord.x = mod(ncoord.y, 1.0) * 2.0;
	} else {
		rectCoord.y = ncoord.y;
		rectCoord.x = mod(ncoord.x, 1.0) * 2.0;
	}
	...
	float lowLevel = texture2D(histTex, vec2(mod(r * 0.42 + specindex, 1.0), 0.0) * 1).r;
...
#define hiLevel 0	
	...
	//a = polarCoord.x;
#else
	a = abs(mod(2.0 * atan(coord2.y, coord2.x)/M_PI + pow(index * spiGain, spiCompress + 1.0) + 4.0 + lowLevel * lowGainA, 2.0) - 1.0);	
#endif
	
	float fcompress_final = fcompress + r * r * fcompressr;
	float amp = texture2D(
			fft_history, 
			vec2(
				freqMin + pow(mod(a , 1.0), 1.0 + fcompress_final) * (freqMax - freqMin),
				1.0 - mod(specindex + (clamp(index + lowLevel * lowGainR, 0.0, 1.0)) / (gain + 1.0), 1.0)
			) * 1
		).r;
		//amp = 1.0;
	...
	gl_FragColor *= vec4(0.9, 2.0, 0.9 , 1.0);
	gl_FragColor.rb *= gl_FragColor.rb;// * gl_FragColor.rgb;
	//gl_FragColor.rgb = min(gl_FragColor.rgb, 0.6);
	...
	gl_FragColor.a = gl_FragColor.a * smoothstep(0.0, smooothExt,1.0 - r) * smoothstep(0.0, smooothInt,r);
		//gl_FragColor.a = r;
