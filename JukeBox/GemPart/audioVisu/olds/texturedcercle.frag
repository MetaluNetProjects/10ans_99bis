...
uniform sampler2DRect fft_history;
uniform sampler2DRect histTex;
...
void main (void)
{
	...
	float lowLevel = texture2DRect(histTex, vec2(mod(r * 0.42 + specindex, 1.0), 0.0) * 1024).r;
...
#define hiLevel 0	
	...
	fcompress += r * fcompressr;
	float amp = texture2DRect(
			fft_history, 
			vec2(
